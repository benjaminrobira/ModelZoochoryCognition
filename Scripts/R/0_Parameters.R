...
parametersValue[3,] <- c("Environment","Period length","Length of a period before a given tree starts producing again", cycleLength, "tu")
parametersValue[4,] <- c("Environment","Number of periods","Number of periods before the simulation is ended", cycleLimitNumber, "")
...
# parametersValue[19,] <- c("","","","")#c("","","Random distribution to obtain the start of fruiting of each tree","")
# parametersValue[20,] <- c("","","","")#c("","",as.character(expression(italic(x)~and~italic(y)~"coordinates of a tree"~italic(i))),"")
